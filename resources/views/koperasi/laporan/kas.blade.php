<x-app-layout>
    @php
        // Normalisasi data bulanan untuk kebutuhan grafik Chart.js
        $monthsArray = [];
        $masukArray = [];
        $keluarArray = [];
        foreach($grafikBulanan as $item) {
            $monthsArray[] = strtoupper(substr($item['bulan'], 0, 3));
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex flex-col">
                <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight leading-tight uppercase">
                    Laporan <span class="text-amber-500">Kas Koperasi</span>
                </h2>
                <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Audit Arus Simpanan & Penarikan Dana Anggota</p>
            </div>
            
            {{-- Filter Tahun Berjalan --}}
            <form action="{{ route('koperasi.laporan.kas') }}" method="GET" class="w-full sm:w-auto">
                <select name="tahun" onchange="this.form.submit()" class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-4 text-xs font-black focus:ring-2 focus:ring-amber-500 cursor-pointer text-slate-700 dark:text-slate-200 uppercase tracking-widest shadow-sm">
    ...
    <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-gradient-to-br from-amber-500 to-orange-600 rounded-[2rem] p-6 shadow-xl shadow-amber-500/10 text-white relative overflow-hidden group">
                ...
                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <p class="text-[10px] font-black text-amber-100 uppercase tracking-[0.2em] mb-1">Total Saldo Mengendap</p>
                        <h1 class="text-3xl font-black tracking-tighter drop-shadow-sm">Rp{{ number_format($saldoAktual, 0, ',', '.') }}</h1>
                    </div>
                    <span class="text-[9px] font-black bg-white/20 px-3 py-1 rounded-md uppercase tracking-wider backdrop-blur-sm">Aktif</span>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-[2rem] p-6 shadow-xl shadow-emerald-500/10 text-white relative overflow-hidden group">
                ...
                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <p class="text-[10px] font-black text-emerald-100 uppercase tracking-[0.2em] mb-1">Total Setoran Masuk</p>
                        <h1 class="text-3xl font-black tracking-tighter drop-shadow-sm">Rp{{ number_format($totalMasuk, 0, ',', '.') }}</h1>
                    </div>
                    <span class="text-[9px] font-black bg-white/20 px-3 py-1 rounded-md uppercase tracking-wider backdrop-blur-sm">Simpanan</span>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-rose-500 to-pink-600 rounded-[2rem] p-6 shadow-xl shadow-rose-500/10 text-white relative overflow-hidden group">
                <svg class="absolute -right-4 -top-4 opacity-15 size-28 transition-transform group-hover:scale-110 duration-300" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <p class="text-[10px] font-black text-rose-100 uppercase tracking-[0.2em] mb-1">Total Penarikan Keluar</p>
                        <h1 class="text-3xl font-black tracking-tighter drop-shadow-sm">Rp{{ number_format($totalKeluar, 0, ',', '.') }}</h1>
                    </div>
                    <span class="text-[9px] font-black bg-white/20 px-3 py-1 rounded-md uppercase tracking-wider backdrop-blur-sm">Penarikan</span>
                </div>
            </div>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-10 gap-6">
            {{-- AREA GRAFIK BAR COMBINATION (6 Kolom) --}}
            <div class="lg:col-span-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 rounded-[2.5rem] shadow-sm flex flex-col justify-between">
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div>
                        <h3 class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Visualisasi Tren Koperasi</h3>
                        <p class="text-[9px] text-slate-400 font-bold uppercase mt-0.5">Grafik Setoran vs Penarikan Dana Anggota THN {{ $tahunIni }}</p>
                    </div>
                </div>
                <div class="relative w-full h-80">
                    <canvas id="chartKoperasiBulanan"></canvas>
                </div>
            </div>

            {{-- AREA LEDGER RINCIAN LIST (4 Kolom) --}}
            <div class="lg:col-span-4 bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
                <div class="flex items-center gap-3 mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div>
                        <h3 class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Ledger Buku Kas Koperasi</h3>
                        <p class="text-[9px] font-bold text-slate-400 uppercase mt-0.5">Rincian Finansial Bulanan</p>
                    </div>
                </div>
                        ...
                        <div class="p-3 rounded-2xl border transition-all flex flex-col gap-2 {{ $adaTransaksi ? 'border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30' : 'border-dashed border-slate-100 dark:border-slate-800 opacity-40 bg-transparent' }}">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-center">
                                <div class="bg-white dark:bg-slate-900 rounded-xl p-1.5 border border-slate-100 dark:border-slate-800/30">
                                    <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Setoran</p>
                                    <p class="text-[10px] font-black mt-0.5 {{ $item['masuk'] > 0 ? 'text-emerald-600' : 'text-slate-400' }}">
                                        Rp{{ number_format($item['masuk'], 0, ',', '.') }}
                                    </p>
                                </div>
                                <div class="bg-white dark:bg-slate-900 rounded-xl p-1.5 border border-slate-100 dark:border-slate-800/30">
                                    <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Penarikan</p>
                                    <p class="text-[10px] font-black mt-0.5 {{ $item['keluar'] > 0 ? 'text-orange-500' : 'text-slate-400' }}">
    ...
    {{-- SCRIPTS GENERATE DIAGRAM KOPERASI --}}
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            const ctxKop = document.getElementById('chartKoperasiBulanan').getContext('2d');
            new Chart(ctxKop, {
                type: 'bar',
                data: {
                    labels: labelsBulan,
