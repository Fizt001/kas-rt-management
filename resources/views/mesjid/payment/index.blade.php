<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col">
            <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight leading-tight">
                Metode <span class="text-emerald-600">Pembayaran</span>
            </h2>
            <p class="text-[11px] text-slate-500 font-medium uppercase tracking-wider italic">Pengaturan Rekening Kas Mesjid</p>
        </div>
    </x-slot>
    ...
    <div class="min-h-screen pb-10">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
            ...
            @if(session('success'))
            <div class="mb-4 bg-emerald-500 text-white px-4 py-3 rounded-xl text-sm font-bold flex items-center gap-3 shadow-lg shadow-emerald-500/20">
            ...
            <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-sm rounded-[2rem] overflow-hidden">
                <!-- Rutenya mengarah ke store yang sudah kita buat -->
                <form action="{{ route('mesjid.payment.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="p-6 lg:p-8 space-y-6">
                        ...
                        <div class="group">
                            ...
                            <input type="text" name="nama_bank" value="{{ $setting->nama_bank ?? '' }}" 
                                class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl py-3.5 px-5 text-sm font-bold focus:ring-2 focus:ring-emerald-500 transition-all dark:text-white" 
                                placeholder="Contoh: BSI, Mandiri, atau DANA">
                        </div>
                    ...
                    <div class="p-6 bg-slate-50/50 dark:bg-slate-800/50 border-t dark:border-slate-800 flex justify-end">
                        <button type="submit" class="w-full sm:w-auto py-3.5 px-10 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-2xl font-black text-sm shadow-xl shadow-emerald-200 dark:shadow-none transition-all active:scale-95">
                            Simpan Perubahan
                        </button>
                    </div>
                </form>
