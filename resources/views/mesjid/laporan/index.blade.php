<x-app-layout>
    @php
        // Ekstrak data bulanan secara otomatis untuk kebutuhan Chart.js
        $monthsArray = [];
        $masukArray = [];
        $keluarArray = [];
        foreach($grafikBulanan as $item) {
            $monthsArray[] = strtoupper($item['bulan']);
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex flex-col">
                <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight leading-tight uppercase">
                    Laporan <span class="text-emerald-600">Keuangan Mesjid</span>
                </h2>
                <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Audit Keuangan Zakat, Infaq & Sedekah Jamaah</p>
            </div>
            
            {{-- Filter Tahun Berjalan --}}
            <form action="{{ url()->current() }}" method="GET" class="w-full sm:w-auto">
                <select name="tahun" onchange="this.form.submit()" class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-4 text-xs font-black focus:ring-2 focus:ring-emerald-500 cursor-pointer text-slate-700 dark:text-slate-200 uppercase tracking-widest shadow-sm">
    ...
    <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[2rem] p-6 shadow-xl shadow-emerald-500/10 text-white relative overflow-hidden group">
                ...
                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <p class="text-[10px] font-black text-emerald-100 uppercase tracking-[0.2em] mb-1">Saldo Kas Aktual</p>
            ...
            <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-[2rem] p-6 shadow-xl shadow-blue-500/10 text-white relative overflow-hidden group">
                ...
                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <p class="text-[10px] font-black text-blue-100 uppercase tracking-[0.2em] mb-1">Total Infaq Masuk</p>
                        <h1 class="text-3xl font-black tracking-tighter drop-shadow-sm">Rp{{ number_format($totalPemasukan, 0, ',', '.') }}</h1>
                    </div>
            ...
            <div class="bg-gradient-to-br from-rose-500 to-pink-600 rounded-[2rem] p-6 shadow-xl shadow-rose-500/10 text-white relative overflow-hidden group">
                ...
                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <p class="text-[10px] font-black text-rose-100 uppercase tracking-[0.2em] mb-1">Total Pengeluaran</p>
                        <h1 class="text-3xl font-black tracking-tighter drop-shadow-sm">Rp{{ number_format($totalPengeluaran, 0, ',', '.') }}</h1>
                    </div>
                    <span class="text-[9px] font-black bg-white/20 px-3 py-1 rounded-md uppercase tracking-wider backdrop-blur-sm">Alokasi</span>
                </div>
            </div>
        ...
        <div class="grid lg:grid-cols-10 gap-6">
            ...
            <div class="lg:col-span-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 rounded-[2.5rem] shadow-sm flex flex-col justify-between">
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div>
                        <h3 class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Visualisasi Tren Kas Mesjid</h3>
                        <p class="text-[9px] text-slate-400 font-bold uppercase mt-0.5">Grafik Perbandingan Infaq Masuk vs Pengeluaran THN {{ $tahunIni }}</p>
                    </div>
                </div>
                {{-- Container Chart --}}
                <div class="relative w-full h-80">
                    <canvas id="chartLaporanBulananMesjid"></canvas>
                </div>
            </div>

            {{-- AREA KANAN: LIST RINCIAN DATA BULANAN (4 Kolom) --}}
            <div class="lg:col-span-4 bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
                <div class="flex items-center gap-3 mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div>
                        <h3 class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Ledger Rincian Bulanan</h3>
                        <p class="text-[9px] font-bold text-slate-400 uppercase mt-0.5">Buku Kas Bulanan Mesjid</p>
                    </div>
                </div>
                        ...
                        <div class="p-3 rounded-2xl border transition-all flex flex-col gap-2 {{ $adaTransaksi ? 'border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30' : 'border-dashed border-slate-100 dark:border-slate-800 opacity-40 bg-transparent' }}">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-center">
                                <div class="bg-white dark:bg-slate-900 rounded-xl p-1.5 border border-slate-100 dark:border-slate-800/30">
                                    <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Infaq</p>
                                ...
                                <div class="bg-white dark:bg-slate-900 rounded-xl p-1.5 border border-slate-100 dark:border-slate-800/30">
                                    ...
                                    <p class="text-[10px] font-black mt-0.5 {{ $item['keluar'] > 0 ? 'text-rose-500' : 'text-slate-400' }}">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            // Parsing data riil dari PHP ke Javascript Array secara aman tanpa error token koma
            ...
            const ctxReport = document.getElementById('chartLaporanBulananMesjid').getContext('2d');
            new Chart(ctxReport, {
                type: 'bar',
                data: {
                    labels: labelsBulan,
