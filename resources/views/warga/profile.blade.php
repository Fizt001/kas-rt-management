    ...
    <div x-data="{ 
        ...
        <div class="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl shadow-sm p-4 sm:p-5 relative overflow-hidden flex flex-col xl:flex-row gap-5 items-start xl:items-center justify-between">
            ...
            <form action="{{ route('warga.profile.update') }}" method="POST" class="relative z-10 flex flex-col lg:flex-row gap-3 w-full xl:w-auto items-end">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full lg:min-w-[550px]">
                    <div class="w-full">
                    ...
                    <div class="w-full">
                        <label class="text-[10px] font-black text-blue-100 uppercase tracking-widest block mb-1">Nomor KK</label>
                        <input type="text" name="no_kk" value="{{ $targetUser->no_kk }}" placeholder="No KK" class="w-full bg-white/10 border border-white/20 text-white placeholder:text-white/40 rounded-lg py-2 px-3 text-xs sm:text-sm font-bold focus:ring-2 focus:ring-white h-10">
                    </div>
                    <div class="w-full">
                        <label class="text-[10px] font-black text-blue-100 uppercase tracking-widest block mb-1">NIK KTP</label>
                        <input type="text" name="nik" value="{{ $targetUser->nik }}" placeholder="NIK Utama" class="w-full bg-white/10 border border-white/20 text-white placeholder:text-white/40 rounded-lg py-2 px-3 text-xs sm:text-sm font-bold focus:ring-2 focus:ring-white h-10">
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 p-4 sm:p-5">
            ...
            <div class="flex justify-between items-center mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
                ...
                <button @click="showModalAdd = true" class="px-4 py-2 bg-slate-800 dark:bg-slate-700 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all flex items-center gap-1.5">
                ...
                <table class="w-full text-left whitespace-nowrap">
                    <thead>
                        <tr class="bg-slate-50 dark:bg-slate-800/50 text-[10px] font-black uppercase tracking-widest text-slate-500">
                            ...
                            <th class="px-4 py-2.5">NIK Anggota</th>
                            <th class="px-4 py-2.5 text-right rounded-r-lg">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($targetUser->familyMembers as $anggota)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                <td class="px-4 py-3 text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">{{ $anggota->nama }}</td>
                                ...
                                <td class="px-4 py-3 text-xs font-bold text-slate-500">{{ $anggota->nik ?? '-' }}</td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex justify-end gap-1.5">
                                        <button type="button" @click="editUrl = '{{ route('warga.family.update', $anggota->id) }}'; editNama = '{{ $anggota->nama }}'; editStatus = '{{ $anggota->status_hubungan }}'; editNik = '{{ $anggota->nik }}'; showModalEdit = true" class="text-blue-600 hover:text-blue-800 bg-blue-50 dark:bg-blue-900/30 p-2 rounded-lg transition-colors">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center">
                                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest italic">Belum ada data anggota keluarga</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                    ...
                    <form action="{{ route('warga.family.store') }}" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select name="status_hubungan" required class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl py-3 px-4 text-sm font-bold focus:ring-2 focus:ring-blue-600 dark:text-white">
