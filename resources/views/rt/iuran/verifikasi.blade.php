<x-app-layout>
    ...
    <div class="p-5 space-y-5 pb-10">
        ...
        <div class="bg-blue-600 dark:bg-slate-800 rounded-xl p-4 shadow-md shadow-blue-200 dark:shadow-none text-white relative overflow-hidden">
            <svg class="absolute -right-4 -top-4 opacity-10 w-24 h-24 sm:w-32 sm:h-32" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm.31-8.86c-1.77-.45-2.34-.94-2.34-1.67 0-.84.79-1.43 2.1-1.43 1.38 0 1.9.66 1.94 1.64h1.71c-.05-1.34-.87-2.57-2.49-2.97V5H10.9v1.69c-1.51.32-2.72 1.3-2.72 2.81 0 1.79 1.49 2.69 3.66 3.21 1.95.46 2.34 1.15 2.34 1.87 0 .53-.39 1.64-2.25 1.64-1.74 0-2.26-.95-2.32-1.81H7.84c.09 1.71 1.25 2.85 3.06 3.2V19h2.33v-1.66c1.64-.32 2.92-1.38 2.92-3.03 0-2.19-1.76-2.8-3.84-3.17z"/></svg>
            
            <h3 class="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-blue-200 mb-2.5">Informasi Nominal Iuran Berlaku</h3>
            <div class="flex flex-wrap gap-2 sm:gap-4 items-end">
                @forelse($masterIurans as $master)
                <div class="bg-white/10 border border-white/20 rounded-lg py-2 px-3 flex-1 min-w-[45%] sm:min-w-[120px] sm:flex-none">
                    <p class="text-[8px] sm:text-[9px] font-bold text-blue-100 uppercase tracking-wider">{{ $master->nama_iuran }}</p>
                    <p class="text-xs sm:text-sm font-black mt-0.5">Rp {{ number_format($master->nominal, 0, ',', '.') }}</p>
                </div>
                @empty
                <div class="bg-white/10 border border-white/20 rounded-lg py-2 px-3 flex-1 sm:flex-none">
                    <p class="text-[8px] sm:text-[9px] font-bold text-blue-100 uppercase tracking-wider">Default Iuran</p>
                    <p class="text-xs sm:text-sm font-black mt-0.5">Rp 0</p>
                </div>
                @endforelse
                
                <div class="w-full sm:w-auto sm:ml-auto bg-yellow-400 text-slate-900 rounded-lg py-2 px-3 shadow-sm mt-1 sm:mt-0">
                    <div class="flex justify-between items-center sm:block">
                        <p class="text-[8px] sm:text-[9px] font-black uppercase tracking-widest opacity-80">Total Bulanan</p>
                        <p class="text-base sm:text-lg font-black sm:mt-0.5">Rp {{ number_format($masterIurans->sum('nominal'), 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- 2. FILTER SECTION -->
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 shadow-sm w-full">
            <form action="{{ route('verifikasi.index') }}" method="GET" class="w-full grid grid-cols-2 sm:flex sm:flex-row gap-2">
                <select name="bulan" class="w-full sm:flex-1 bg-slate-50 dark:bg-slate-800 border-none rounded-lg py-2 px-3 text-[10px] sm:text-xs font-bold focus:ring-2 focus:ring-blue-600 dark:text-white uppercase tracking-wider cursor-pointer">
                ...
                <select name="tahun" class="w-full sm:flex-1 bg-slate-50 dark:bg-slate-800 border-none rounded-lg py-2 px-3 text-[10px] sm:text-xs font-bold focus:ring-2 focus:ring-blue-600 dark:text-white uppercase tracking-wider cursor-pointer">
                    <option value="">Semua Tahun</option>
                    <option value="2026" {{ request('tahun') == '2026' ? 'selected' : '' }}>2026</option>
                    <option value="2027" {{ request('tahun') == '2027' ? 'selected' : '' }}>2027</option>
                </select>

                <button type="submit" class="w-full col-span-2 sm:col-span-1 sm:w-auto bg-slate-800 text-white px-6 py-2 rounded-lg text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-slate-700 transition-all shadow-sm active:scale-95">
                    Filter
                </button>
            </form>
        </div>

        <!-- 3. KONTEN UTAMA VERIFIKASI (DESKTOP & MOBILE) -->
        <div x-data="{ imgModal: false, imgSrc: '' }">
            <!-- Desktop View: TABEL VERIFIKASI -->
            ...
            <!-- Mobile View: ACCORDION VERIFIKASI -->
            <div class="block lg:hidden space-y-3">
                ...
                <div x-data="{ expanded: false }" class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm transition-all duration-300">
                    ...
                    <div @click="expanded = !expanded" class="p-4 flex items-center justify-between cursor-pointer active:bg-slate-50 dark:active:bg-slate-800/50">
                        <div class="flex-1 min-w-0 pr-4">
                            <h3 class="text-xs font-black text-slate-800 dark:text-slate-200 truncate uppercase">{{ $pay->user->name ?? 'Warga' }}</h3>
                            <p class="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                                {{ [1=>'Jan',2=>'Feb',3=>'Mar',4=>'Apr',5=>'Mei',6=>'Jun',7=>'Jul',8=>'Ags',9=>'Sep',10=>'Okt',11=>'Nov',12=>'Des'][$pay->bulan] ?? 'Bulan '.$pay->bulan }} {{ $pay->tahun }}
                            </p>
                        </div>
                    ...
                    <div x-show="expanded" x-collapse x-cloak>
                        <div class="p-4 pt-0 border-t border-slate-100 dark:border-slate-800 mt-2">
                            <div class="flex flex-col sm:flex-row gap-3 mt-4">
                                <!-- Tombol Lihat -->
                                <button @click="imgSrc = '{{ asset('storage/' . $pay->bukti_transfer) }}'; imgModal = true" type="button" class="w-full sm:w-1/2 flex justify-center items-center gap-2 px-3 py-2 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase hover:bg-blue-100 transition-colors border border-blue-100 shadow-sm">
                                    <svg class="size-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"/></svg>
                                    Lihat Bukti TRF
                                </button>
                                
                                <!-- Tombol Setujui -->
                                <form id="form-approve-mob-{{ $pay->id }}" action="{{ route('verifikasi.approve', $pay->id) }}" method="POST" class="w-full sm:w-1/2">
                                    @csrf
                                    <button type="button" onclick="confirmApproval('{{ $pay->id }}', '{{ addslashes($pay->user->name ?? 'Warga') }}', '{{ [1=>'Januari',2=>'Februari',3=>'Maret',4=>'April',5=>'Mei',6=>'Juni',7=>'Juli',8=>'Agustus',9=>'September',10=>'Oktober',11=>'November',12=>'Desember'][$pay->bulan] ?? 'Bulan '.$pay->bulan }}', '{{ $pay->tahun }}')" class="w-full py-2 px-4 bg-emerald-500 text-white text-[10px] uppercase tracking-widest font-black rounded-lg hover:bg-emerald-600 shadow-sm shadow-emerald-200 transition-all active:scale-95">
                                        Setujui
                                    </button>
                                </form>
                            </div>
                ...
                <div class="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
                    <p class="text-[10px] font-black uppercase text-slate-300 tracking-[0.2em]">Tidak Ada Pembayaran Menunggu</p>
                </div>
                @endforelse
            </div>

            <div class="mt-4 px-6 py-4 bg-slate-50/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-xl flex justify-center">
            ...
            <div x-show="imgModal" @keydown.window.escape="imgModal = false" style="display: none;" class="fixed inset-0 z-[100] overflow-y-auto">
                ...
                <div class="flex items-center justify-center min-h-screen p-4 text-center z-10 relative">
                    <div x-show="imgModal" 
                         ...
                         class="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-xl w-full p-2">
